...
parser.add_argument('--batch-size', default=1, type=int, help='Minibatch size')
...
def main():
    ...
    for n, p in model.named_parameters():
        model_dict[n] = p
        if "conv.weight" in n and not "conv.weight_quant.tensor_quant" in n:
            w_mean[n+"_quant.tensor_quant.s"] = torch.nn.Parameter(torch.ones(1)*p.abs().mean()*5)
    
    for n, p in model_dict.items():
        if "conv.weight_quant.tensor_quant" in n:
            model_dict[n] = w_mean[n]
